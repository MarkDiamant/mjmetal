...
export default function NewJobForm() {
  ...
  useEffect(() => {
    fetch("/api/demo/customers", { cache: "no-store" }).then(async (response) => {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/demo/jobs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    if (response.status === 401) { router.replace("/demo/login"); return; }
    const result = await response.json().catch(() => ({}));
    if (!response.ok) { setError(result.error || "Unable to create job"); setSaving(false); return; }
    router.push(`/demo/jobs/${result.job.reference}`); router.refresh();
  }

  const selected = customers.find((customer) => customer.id === selectedCustomerId);

  return <main className="min-h-screen bg-[#f5f5f2] text-[#141414]"><div className="mx-auto max-w-5xl px-5 py-7 lg:px-8"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#e66a24]">Demo Metal CRM</p><h1 className="mt-1 text-3xl font-black">New job</h1><p className="mt-2 text-sm text-black/55">The next MJ reference is allocated automatically when you create the job.</p></div><a href="/demo" className="rounded-xl border border-black/15 bg-white px-4 py-2.5 text-sm font-bold">Back</a></div>
