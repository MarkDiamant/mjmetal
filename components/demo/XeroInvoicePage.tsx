...
export default function XeroInvoicePage({ reference }: { reference: string }) {
  ...
  const load = useCallback(async () => {
    ...
    const [jobRes, xeroRes] = await Promise.all([
      fetch(`/api/demo/jobs/${encodeURIComponent(reference)}`, { cache: "no-store" }),
      fetch(`/api/demo/jobs/${encodeURIComponent(reference)}/xero`, { cache: "no-store" }),
    ]);
    if (jobRes.status === 401 || xeroRes.status === 401) { window.location.href = "/demo/login"; return; }
  ...
  useEffect(() => { void load(); fetch("/api/demo/settings",{cache:"no-store"}).then(async r=>{if(r.ok){const b=await r.json();if(b.settings)setCrmConfig(b.settings);}}).catch(()=>{}); }, [load]);
  ...
  async function action(actionName: "create" | "sync") {
    ...
    const res = await fetch(`/api/demo/jobs/${encodeURIComponent(reference)}/xero`, {
      method: "POST",
  ...
  return <main className={`min-h-screen bg-[#f5f5f2] px-5 py-8 text-[#141414] ${invoiceClass}`} style={{"--invoice-accent":invoiceAccent} as React.CSSProperties}>
    <style>{`@media print{@page{size:A4;margin:0}body{background:white}.invoice-controls{display:none!important}.invoice-sheet{display:flex!important;width:210mm;min-height:297mm;margin:0!important;box-shadow:none!important;border:0!important}}`}</style><div className="mx-auto max-w-4xl">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="invoice-controls flex gap-2">{invoices.length>0&&<button type="button" onClick={()=>window.print()} className="rounded-xl bg-[var(--invoice-accent)] px-4 py-2.5 text-sm font-black text-white">Print / Save invoice PDF</button>}<Link href={`/demo/jobs/${reference}`} className="rounded-xl border border-black/15 bg-white px-4 py-2.5 text-sm font-bold">Back to job</Link></div>
      </div>
