...
import CrmDashboardV3 from "@/components/demo/CrmDashboardV3";
    ...
    const applyActivityTimes = (activities: DashboardActivity[]) => {
      ...
      for (const activity of extra) {
        ...
        link.href = activity.mj_jobs?.reference
          ? `/demo/jobs/${encodeURIComponent(activity.mj_jobs.reference)}`
          : "/demo";
      ...
      for (const [personName, payments] of groups) {
        ...
        for (const payment of payments) {
          ...
          edit.onclick = async (event) => {
            ...
            await fetch(
              `/api/demo/jobs/${encodeURIComponent(reference)}/payments/${encodeURIComponent(payment.id)}`,
              {
                method: "PATCH",
    ...
    const refresh = async () => {
      ...
      try {
        ...
        if (!dashboardActivities.length || now - lastDashboardFetch > 5000) {
          const dashboardResponse = await fetch("/api/demo/dashboard", { cache: "no-store" });
        ...
        for (const heading of headings) {
          ...
          const paymentResponse = await fetch(
            `/api/demo/jobs/${encodeURIComponent(reference)}/subcontractor-payments`,
            { cache: "no-store" },
          );
