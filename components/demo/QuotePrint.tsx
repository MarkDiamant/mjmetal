...
export default function QuotePrint({ reference, quoteId }: { reference: string; quoteId: string }) {
  ...
  useEffect(()=>{fetch("/api/demo/settings",{cache:"no-store"}).then(async r=>{if(r.ok){const b=await r.json();if(b.settings)setCrmConfig(b.settings);}}).catch(()=>{});fetch("/api/integrations/google/status",{cache:"no-store"}).then(async r=>{if(r.ok){const b=await r.json();if(b.connected&&b.email)setConnectedGmail(String(b.email));}}).catch(()=>{});},[]);
  useEffect(() => {
    fetch(`/api/demo/jobs/${encodeURIComponent(reference)}`, { cache: "no-store" }).then(async (res) => {
      if (res.status === 401) { window.location.href = "/demo/login"; return; }
...
${crmConfig.businessName}`;
  ...
  async function sendQuote() {
    ...
    const response = await fetch(`/api/demo/jobs/${encodeURIComponent(reference)}/send-quote`, {
      method: "POST",
  ...
  return <><main className="min-h-screen bg-[#ecece8] py-6 text-[#171717] print:bg-white print:py-0" style={{"--brand":crmConfig.accentColour} as React.CSSProperties}>
    ...
    <div className="mx-auto mb-4 flex max-w-[900px] flex-wrap justify-end gap-2 px-4 print:hidden">
      ...
      <a href={`/demo/jobs/${reference}`} className="rounded-xl border border-black/15 bg-white px-4 py-2.5 text-sm font-bold">Back to job</a><a href="/demo" className="rounded-xl border border-black/15 bg-white px-4 py-2.5 text-sm font-bold">Back to admin</a>
    </div>
