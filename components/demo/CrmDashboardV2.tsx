          ...
          <div className="mt-4 space-y-2">
            {filtered.map((j)=> <div key={j.id} className={`overflow-hidden rounded-2xl border bg-white shadow-[0_5px_20px_rgba(0,0,0,0.025)] ${editingRef===j.reference?"border-[#e66a24]/35":"border-black/8"}`}>
              ...
              {editingRef===j.reference&&draft&&<div className="border-t border-[#e66a24]/20 bg-[#fffaf6] p-4 sm:p-5">
                <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.12em] text-[#e66a24]">Quick edit</p><h2 className="text-xl font-black">{j.reference}</h2></div><div className="flex flex-wrap gap-2"><button onClick={()=>void saveQuick(j,{status:"completed",completed_at:new Date().toISOString()})} className="rounded-xl border border-green-200 bg-green-50 px-4 py-2 text-sm font-black text-green-800">Mark complete</button><button onClick={()=>void archiveJob(j)} className="rounded-xl border border-black/15 bg-white px-4 py-2 text-sm font-bold">Archive</button><a href={`/demo/jobs/${j.reference}`} className="rounded-xl border border-black/15 bg-white px-4 py-2 text-sm font-bold">Open full job</a><button disabled={savingRef===j.reference} onClick={()=>void saveQuick(j)} className="rounded-xl bg-[#141414] px-5 py-2 text-sm font-black text-white disabled:opacity-50">{savingRef===j.reference?"Saving...":"Save changes"}</button></div></div>
