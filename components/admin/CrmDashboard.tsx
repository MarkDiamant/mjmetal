...
export default function CrmDashboard() {
  ...
  return <main className="min-h-screen bg-[#f5f5f2] text-[#141414]">
    <header className="border-b border-black/10 bg-white"><div className="mx-auto flex max-w-[1500px] items-center justify-between px-5 py-4 lg:px-8"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#e66a24]">M&J Metal</p><h1 className="text-2xl font-black tracking-tight">CRM & Job Management</h1></div><div className="flex items-center gap-3">{adminName && <span className="hidden rounded-full bg-[#f3f3ef] px-3 py-2 text-sm font-semibold sm:inline">{adminName}</span>}<Link href="/admin/integrations" className="hidden rounded-xl border border-black/15 bg-white px-3 py-2.5 text-sm font-bold md:block">Integrations</Link><button onClick={logout} className="hidden rounded-xl border border-black/15 bg-white px-3 py-2.5 text-sm font-bold md:block">Logout</button><Link href="/admin/jobs/new" className="rounded-xl bg-[#e66a24] px-4 py-2.5 text-sm font-black text-white shadow-sm">+ New job</Link></div></div></header>
