    ...
    <header className="border-b border-black/10 bg-white">
      <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4 px-5 py-4 lg:px-8">
        <div className="flex min-w-0 items-center gap-4">
          <img src="/images/logo.png" alt="M&J Metal" className="h-14 w-auto shrink-0 object-contain sm:h-16" />
          <div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#e66a24]">M&J Metal</p><h1 className="truncate text-xl font-black tracking-tight sm:text-2xl">CRM & Job Management</h1></div>
        </div>
        <div className="flex items-center gap-3">{adminName && <span className="hidden rounded-full bg-[#f3f3ef] px-3 py-2 text-sm font-semibold sm:inline">{adminName}</span>}<Link href="/admin/integrations" className="hidden rounded-xl border border-black/15 bg-white px-3 py-2.5 text-sm font-bold md:block">Integrations</Link><button onClick={logout} className="hidden rounded-xl border border-black/15 bg-white px-3 py-2.5 text-sm font-bold md:block">Logout</button><Link href="/admin/jobs/new" className="rounded-xl bg-[#e66a24] px-4 py-2.5 text-sm font-black text-white shadow-sm">+ New job</Link></div>
      </div>
    </header>
