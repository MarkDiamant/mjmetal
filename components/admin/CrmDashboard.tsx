...
export default function CrmDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!response.ok) {
        if (!cancelled) { setError(data.error || "Unable to load jobs"); setLoading(false); }
        return;
      }
      if (!cancelled) {
        setJobs(data.jobs || []);
        setAdminName(data.admin?.display_name || "");
        setLoading(false);
      }
    }
    load();
  ...
  }, [jobs, manager, query, status, type]);
    ...
    <main className="min-h-screen bg-[#f5f5f2] text-[#141414]">
      <header className="border-b border-black/10 bg-white">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between px-5 py-4 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            {adminName && <span className="hidden rounded-full bg-[#f3f3ef] px-3 py-2 text-sm font-semibold sm:inline">{adminName}</span>}
            <button onClick={logout} className="hidden rounded-xl border border-black/15 bg-white px-3 py-2.5 text-sm font-bold md:block">Logout</button>
            <Link href="/admin/jobs/new" className="rounded-xl bg-[#e66a24] px-4 py-2.5 text-sm font-black text-white shadow-sm">+ New job</Link>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-[1500px] px-5 py-6 lg:px-8">
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[["Active jobs", counts.active.toString()], ["Follow-ups due", counts.followUps.toString()], ["Scheduled", counts.scheduled.toString()], ["Outstanding", money(counts.outstanding)]].map(([label, value]) => (
            <div key={label} className="rounded-2xl border border-black/10 bg-white p-5 shadow-[0_8px_30px_rgba(0,0,0,0.04)]">
              <p className="text-sm font-semibold text-black/50">{label}</p><p className="mt-2 text-3xl font-black">{value}</p>
            </div>
          ))}
        </section>
        ...
        <section className="mt-6 rounded-2xl border border-black/10 bg-white p-4 shadow-[0_8px_30px_rgba(0,0,0,0.04)]">
          <div className="grid gap-3 lg:grid-cols-[minmax(240px,1fr)_220px_180px_220px]">
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search ref, customer, address, postcode, phone..." className="h-11 rounded-xl border border-black/15 bg-white px-3 text-sm outline-none focus:border-[#e66a24]" />
            <select value={status} onChange={(e) => setStatus(e.target.value as JobStatus | "all")} className="h-11 rounded-xl border border-black/15 px-3 text-sm"><option value="all">All statuses</option>{ALL_STATUSES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
            <select value={manager} onChange={(e) => setManager(e.target.value as Manager | "all")} className="h-11 rounded-xl border border-black/15 px-3 text-sm"><option value="all">Mark + Jonathan</option><option value="MD">Mark</option><option value="JB">Jonathan</option></select>
            <select value={type} onChange={(e) => setType(e.target.value)} className="h-11 rounded-xl border border-black/15 px-3 text-sm"><option value="all">All job types</option>{JOB_TYPES.map((jobType) => <option key={jobType}>{jobType}</option>)}</select>
          </div>
        </section>
              ...
              <tbody>
                {filteredJobs.map((job) => (
                  <tr key={job.id} className="border-t border-black/8 align-top hover:bg-[#fffaf6]">
                    ...
                    <td className="px-4 py-4"><Link href={`/admin/jobs/${job.id}`} className="rounded-lg border border-black/15 px-3 py-2 text-sm font-bold">Open</Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          {loading && <div className="p-10 text-center text-sm text-black/50">Loading jobs...</div>}
          {!loading && filteredJobs.length === 0 && <div className="p-10 text-center text-sm text-black/50">No jobs match these filters.</div>}
        </section>
      </div>
    </main>
  );
