...
export default function QuotePrint({ reference, quoteId }: { reference: string; quoteId: string }) {
  ...
  return <main className="min-h-screen bg-[#ecece8] py-6 text-[#171717] print:bg-white print:py-0" style={{"--brand":crmConfig.accentColour} as React.CSSProperties}>
    ...
    <article className={`quote-page ${pageClass} mx-auto flex min-h-[1120px] max-w-[900px] flex-col bg-white px-10 py-9 shadow-xl sm:px-14 ${isMj&&photos.length===0?"hidden":""}`}>
      ...
      {isMj ? photos.length > 0 && <section className="mt-8"><p className="mb-3 text-[10px] font-black uppercase tracking-[0.12em] text-[var(--brand)]">Project Images</p><div className={`grid gap-4 ${photos.length===1?"grid-cols-1":photos.length===2?"grid-cols-2":"grid-cols-3"}`}>{photos.map((f: any) => <img key={f.id} src={f.signed_url} alt={f.file_name} className={`${photos.length===1?"h-[520px]":"h-64"} w-full rounded-lg border border-black/10 object-cover`}/>)}</div></section> : photos.length > 0 && <section className="mt-6"><div className={`grid gap-3 ${photos.length===1?"grid-cols-1":photos.length===2?"grid-cols-2":"grid-cols-3"}`}>{photos.map((f: any) => <img key={f.id} src={f.signed_url} alt={f.file_name} className="h-48 w-full rounded-lg border border-black/10 object-cover" />)}</div></section>}
    ...
    <article className={`quote-page ${pageClass} mx-auto ${isMj&&photos.length===0?"mt-0":"mt-6"} flex min-h-[1120px] max-w-[900px] flex-col bg-white px-10 py-9 shadow-xl print:mt-0 sm:px-14`}>
      <header className={`flex items-center justify-between ${isClassic?"border-b border-black pb-4":isMj?"border-b-4 border-[var(--brand)] pb-4":"border-b-2 border-[var(--brand)] pb-4"}`}><img src={crmConfig.logoUrl} alt={crmConfig.businessName} className="h-14 w-auto object-contain"/><div className="text-right"><p className="font-black">{isMj ? j.reference : `${j.reference} / V${quote.version}`}</p><p className="text-xs text-black/45">Quotation details</p></div></header>{isMj&&photos.length===0&&<section className="mt-5 grid gap-6 sm:grid-cols-2"><div><p className="text-[10px] font-black uppercase tracking-[0.12em] text-[var(--brand)]">Client Information</p><p className="mt-1 text-base font-black">{name}</p>{site&&<p className="mt-1 text-xs leading-5 text-black/65">{site}</p>}{c.phone&&<p className="mt-1 text-xs text-black/55">{c.phone}</p>}{c.email&&<p className="text-xs text-black/55">{c.email}</p>}</div><div><p className="text-[10px] font-black uppercase tracking-[0.12em] text-[var(--brand)]">Project</p><p className="mt-1 text-base font-black">{j.job_type}</p><p className="mt-1 text-xs text-black/55">Issued {new Date(quote.created_at).toLocaleDateString("en-GB")}{quote.valid_until?` · Valid until ${new Date(`${quote.valid_until}T12:00:00`).toLocaleDateString("en-GB")}`:""}</p></div></section>}
      ...
      <section className="mt-6"><h2 className="text-base font-black">{isMj ? "Payment Terms & Details" : "Payment & key terms"}</h2><div className="mt-3 grid gap-x-8 gap-y-2 text-[12px] leading-5 text-black/65 sm:grid-cols-2">
        ...
        <p><b>Ownership:</b> Fabricated items remain the property of the supplier until paid for in full.</p>{isMj&&crmConfig.businessDetails.bankName&&<div className="sm:col-span-2 mt-1 rounded-lg border border-black/10 bg-[#fafaf8] px-3 py-2.5 text-[12px] leading-5 text-black/70"><b className="text-black">Bank details:</b> Account name: {crmConfig.businessDetails.bankName} · Sort code: {crmConfig.businessDetails.sortCode} · Account number: {crmConfig.businessDetails.accountNumber}</div>}
      </div></section>
