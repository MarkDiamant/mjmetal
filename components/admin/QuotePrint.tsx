...
export default function QuotePrint({ reference, quoteId }: { reference: string; quoteId: string }) {
  ...
  return <main className="min-h-screen bg-[#ecece8] py-6 text-[#171717] print:bg-white print:py-0" style={{"--brand":crmConfig.accentColour} as React.CSSProperties}>
    ...
    <article className="quote-page mx-auto mt-6 flex min-h-[1120px] max-w-[900px] flex-col bg-white px-10 py-9 shadow-xl print:mt-0 sm:px-14">
      ...
      <footer className="mt-auto border-t border-black/10 pt-4 text-[10px] leading-4 text-black/45"><p className="font-bold text-black/60">{crmConfig.businessName.toUpperCase()}</p><p>{crmConfig.businessDetails.officeAddress}</p><p>{[crmConfig.businessDetails.phone,crmConfig.businessDetails.email,crmConfig.businessDetails.website,crmConfig.businessDetails.companyNumber?`Company No. ${crmConfig.businessDetails.companyNumber}`:""].filter(Boolean).join(" · ")}</p></footer>
    </article>
  </main>;
}
