...
export default function QuotePrint({ reference, quoteId }: { reference: string; quoteId: string }) {
  ...
  return <main className="min-h-screen bg-[#ecece8] py-6 text-[#171717] print:bg-white print:py-0" style={{"--brand":crmConfig.accentColour} as React.CSSProperties}>
    ...
    <article className={`quote-page ${pageClass} mx-auto flex min-h-[1120px] max-w-[900px] flex-col bg-white px-10 py-9 shadow-xl sm:px-14`}>
      ...
      {isMj ? <section className="mt-8 min-h-[470px]"><p className="mb-3 text-[10px] font-black uppercase tracking-[0.12em] text-[var(--brand)]">Project Images</p>{photos.length > 0 ? <div className={`grid gap-4 ${photos.length===1?"grid-cols-1":photos.length===2?"grid-cols-2":"grid-cols-3"}`}>{photos.map((f: any) => <img key={f.id} src={f.signed_url} alt={f.file_name} className="h-64 w-full rounded-lg border border-black/10 object-cover" />)}</div> : <div className="h-[390px] rounded-xl border border-black/10 bg-[#fafaf8]" />}</section> : photos.length > 0 && <section className="mt-6"><div className={`grid gap-3 ${photos.length===1?"grid-cols-1":photos.length===2?"grid-cols-2":"grid-cols-3"}`}>{photos.map((f: any) => <img key={f.id} src={f.signed_url} alt={f.file_name} className="h-48 w-full rounded-lg border border-black/10 object-cover" />)}</div></section>}
