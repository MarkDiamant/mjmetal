    ...
    <div className="sticky top-0 z-30 border-b border-black/10 bg-white/95 backdrop-blur print:hidden">
      <div className="mx-auto flex max-w-[1500px] flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:px-5 lg:px-8">
        <a href="/admin" className="flex min-w-0 items-center gap-2.5 sm:gap-3">
          ...
          <div className="min-w-0"><p className="text-xs font-black uppercase tracking-[0.12em] sm:tracking-[0.16em]" style={{color:config.accentColour}}>{config.businessName}</p><p className="truncate text-xs font-bold text-black/55 sm:text-sm">{config.systemName}</p><p className="mt-0.5 text-[10px] font-bold text-black/35">Powered by <span className="font-black text-[#e66a24]">Diamant Solutions</span></p></div>
        </a>
