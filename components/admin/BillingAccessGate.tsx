...
export default function BillingAccessGate({children}:{children:React.ReactNode}){
  ...
  const check=useCallback(()=>{fetch("/api/admin/billing-access",{cache:"no-store"}).then(async r=>{
    ...
    if(r.status===401){if(window.location.pathname!=="/admin/login")window.location.replace("/admin/login");return;}
  ...
  useEffect(()=>{if(window.location.pathname!=="/admin/login")check();},[check]);
  if(typeof window!=="undefined"&&window.location.pathname==="/admin/login")return <>{children}</>;
