...
export default function BillingAccessGate({children}:{children:React.ReactNode}){
  ...
  const check=useCallback(()=>{setState(null);fetch("/api/admin/billing-access",{cache:"no-store"}).then(async r=>{
    ...
    if(r.ok)setState(body);else setState({error:true,reason:body.reason,message:body.message||"We couldn’t open your CRM just now. Please try again."});
  ...
  if(state.error)return <main className="min-h-[60vh] bg-[#e7e7e4] px-5 py-16"><div className="mx-auto max-w-lg rounded-3xl border border-black/10 bg-white p-7 text-center shadow-xl"><h1 className="text-2xl font-black">{state.reason==="signed_out"?"Please sign in again":"We couldn’t open your CRM"}</h1><p className="mt-3 text-sm leading-6 text-black/60">{state.message||"Something went wrong while opening your account. Your subscription has not been blocked. Please try again."}</p><button onClick={check} className="mt-6 rounded-xl bg-black px-5 py-3 text-sm font-black text-white">Try again</button></div></main>;
