...
export default function BillingAccessGate({children}:{children:React.ReactNode}){
  ...
  const check=useCallback(()=>{setState(null);fetch("/api/admin/billing-access",{cache:"no-store"}).then(async r=>{
    ...
    if(r.ok)setState(body);else setState({error:true,message:body.message||"There was a temporary problem opening your CRM. Please try again."});
  }).catch(()=>setState({error:true,message:"There was a temporary problem opening your CRM. Please try again."}));},[]);
  ...
  if(state.error)return <main className="min-h-[60vh] bg-[#e7e7e4] px-5 py-16"><div className="mx-auto max-w-lg rounded-3xl border border-black/10 bg-white p-7 text-center shadow-xl"><h1 className="text-2xl font-black">Your CRM is temporarily unavailable</h1><p className="mt-3 text-sm leading-6 text-black/60">{state.message||"There was a temporary problem opening your CRM. This is not a payment warning. Please try again in a moment."}</p><button onClick={check} className="mt-6 rounded-xl bg-black px-5 py-3 text-sm font-black text-white">Try again</button></div></main>;
