...
export default function NewJobForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      siteVisitRequired,
      preliminaryEstimate: data.preliminaryEstimate || null,
    ...
    <main className="min-h-screen bg-[#f5f5f2] text-[#141414]">
      <div className="mx-auto max-w-5xl px-5 py-7 lg:px-8">
        <div className="flex items-start justify-between gap-4">
          <div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#e66a24]">M&J Metal CRM</p><h1 className="mt-1 text-3xl font-black">New job</h1><p className="mt-2 text-sm text-black/55">The next MJ reference is allocated automatically when you create the job.</p></div>
        ...
        <form className="mt-6 space-y-5" onSubmit={submit}>
          <Section title="Customer">
            ...
            {existingCustomer && <Field label="Existing customer" wide><select value={selectedCustomerId} onChange={(e) => setSelectedCustomerId(e.target.value)} required><option value="">Select customer</option>{customers.map((c) => <option key={c.id} value={c.id}>{c.firstName} {c.lastName} {c.postcode ? `• ${c.postcode}` : ""}</option>)}</select></Field>}
            {existingCustomer && selected ? <div className="sm:col-span-2 lg:col-span-3 rounded-xl bg-[#f5f5f2] p-4 text-sm"><b>{selected.firstName} {selected.lastName}</b><br />{selected.addressLine1} {selected.addressLine2} {selected.city} {selected.postcode}<br />{selected.phone} {selected.email}</div> : !existingCustomer && <>
              <Field label="First name"><input name="firstName" required /></Field><Field label="Surname"><input name="lastName" /></Field><Field label="Phone"><input name="phone" inputMode="tel" /></Field><Field label="Email"><input name="email" type="email" /></Field><Field label="Address line 1"><input name="addressLine1" /></Field><Field label="Address line 2"><input name="addressLine2" /></Field><Field label="City"><input name="city" defaultValue="London" /></Field><Field label="Postcode"><input name="postcode" /></Field>
            </>}
          </Section>

          <Section title="Job">
            <Field label="Job type"><select name="jobType">{JOB_TYPES.map((item) => <option key={item}>{item}</option>)}</select></Field>
            <Field label="Managed by"><select name="manager"><option value="MD">Mark</option><option value="JB">Jonathan</option></select></Field>
            <Field label="Enquiry source"><select name="source">{sources.map((item) => <option key={item}>{item}</option>)}</select></Field>
            <Field label="Enquiry date / time"><input name="enquiryAt" type="datetime-local" /></Field>
            <Field label="Approx dimensions"><input name="dimensions" placeholder="e.g. 3.8m wide x 1.2m high" /></Field>
            <Field label="Material"><select name="material"><option>Mild steel</option><option>Stainless steel</option><option>Aluminium</option><option>Other</option></select></Field>
            <Field label="Colour / RAL"><input name="colour" placeholder="e.g. Black / RAL 9005" /></Field>
            ...
            <Field label="Customer requirements" wide><textarea name="customerRequirements" rows={4} placeholder="What the customer wants, design, locks, access, special requirements..." /></Field>
            <Field label="Internal job notes" wide><textarea name="internalNotes" rows={4} placeholder="Measurements, pricing thoughts, subcontractor notes, anything not customer-facing..." /></Field>
          </Section>
          ...
          <Section title="Site visit and next action">
            ...
            {siteVisitRequired && <Field label="Site visit date / time"><input name="siteVisitAt" type="datetime-local" /></Field>}
            <Field label="Next action"><input name="nextAction" placeholder="e.g. Call customer to arrange survey" /></Field><Field label="Next action date / time"><input name="nextActionAt" type="datetime-local" /></Field>
          </Section>

          <Section title="Initial commercial details">
            <Field label="Preliminary estimate"><input name="preliminaryEstimate" type="number" min="0" step="0.01" placeholder="£" /></Field><Field label="Expected quote value"><input name="quotedAmount" type="number" min="0" step="0.01" placeholder="£" /></Field><Field label="Expected payment method"><select name="paymentMethod"><option>Bank transfer</option><option>Cash</option><option>Card</option><option>Other</option></select></Field>
          </Section>
