...
export default function ArchivePage() {
  ...
  async function load() {
    setLoading(true);
    const response = await fetch("/api/demo/jobs?archived=1", { cache: "no-store" });
  ...
  async function restore(reference: string) {
    const response = await fetch(`/api/demo/jobs/${encodeURIComponent(reference)}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ job: { archived_at: null } }) });
