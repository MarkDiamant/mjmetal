...
export async function POST(request: Request) {
  try {
    ...
    const authResponse = await supabaseRequest("/auth/v1/token?grant_type=password", {method:"POST",body:JSON.stringify({ email, password })});
    if (!authResponse.ok) return NextResponse.json({ error: "Incorrect email or password" }, { status: 401 });
    const session = await authResponse.json() as {access_token:string;refresh_token:string;expires_in:number;user:{id:string;email?:string}};
    const adminResponse = await supabaseRequest(`/rest/v1/mj_admin_users?user_id=eq.${encodeURIComponent(session.user.id)}&select=display_name,initials&limit=1`,{method:"GET"},session.access_token);
