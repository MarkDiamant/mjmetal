...
export async function POST(request: Request, { params }: { params: Promise<{ reference: string }> }) {
  ...
  await Promise.all([
    supabaseRequest(`/rest/v1/mj_quotes?id=eq.${encodeURIComponent(quote.id)}`, { method: "PATCH", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ status: "sent", pdf_path: storagePath }) }, session.token),
    supabaseRequest(`/rest/v1/mj_jobs?id=eq.${encodeURIComponent(job.id)}`, { method: "PATCH", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ status: "quote_sent", quote_sent_at: sentAt, next_action: "Follow up quote", next_action_at: new Date(Date.now()+3*86400000).toISOString(), updated_at: sentAt }) }, session.token),
