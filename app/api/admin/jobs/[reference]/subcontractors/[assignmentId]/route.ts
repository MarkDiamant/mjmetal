...
export async function PATCH(request: Request, { params }: { params: Promise<{ reference: string; assignmentId: string }> }) {
  ...
  try {
    ...
    if (addPayment > 0 && !("paid_amount" in patch)) {
      patch.paid_amount = Number(assignment.paid_amount || 0) + addPayment;
    }
    ...
    if (addPayment > 0) {
      await supabaseRequest("/rest/v1/mj_payments", {
        ...
        body: JSON.stringify({
          ...
          payment_type: body.payment_type || "Subcontractor payment",
          amount: addPayment,
          payment_method: body.payment_method || "Bank transfer",
          counterparty: body.counterparty || null,
          paid_at: body.paid_at || new Date().toISOString(),
          notes: `Assignment ${assignmentId}`,
        }),
      }, session.token);
    }
    ...
    await supabaseRequest("/rest/v1/mj_audit_events", {
      ...
      body: JSON.stringify({
        job_id: job.id,
        actor: session.admin.initials,
        action: "updated",
        entity_type: "subcontractor_assignment",
        entity_id: assignmentId,
        changes: { ...patch, add_payment: addPayment || undefined, paid_at: body.paid_at || undefined },
      }),
    }, session.token);
