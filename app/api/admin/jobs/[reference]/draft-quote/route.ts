...
export async function POST(request:Request,{params}:{params:Promise<{reference:string}>}){
 ...
 const html=`<div style="font-family:Arial,sans-serif;line-height:1.6"><p>Dear ${esc(customer.first_name||name)},</p><p>Please find attached our quotation <strong>${esc(job.reference)}</strong>.</p><p>If you have any questions, simply reply to this email.</p><p>Kind regards,<br><strong>${esc(config.businessName)}</strong><br>${esc(config.businessDetails.email)}</p></div>`;
